...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['PET_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['PET_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Pet API.
 */
export class Pet {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['PET_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('PET_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PetError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Pet client with an apiKey option, like new Pet({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Pet.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PET_LOG'), "process.env['PET_LOG']", this) ??
      defaultLogLevel;
  ...
  static PetError = Errors.PetError;
